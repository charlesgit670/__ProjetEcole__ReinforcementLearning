...
# initialize first player => if True, bot play first
play_first = False
policy_path = 'policy/' + 'policy_play_first_' + str(play_first) + '.json'
...
def draw_status():
	...
	if winner is None:
		if (play_first and XO == 1) or (not play_first and XO == 2):
			message = "bot turn"
		else:
			message = "player turn"
	else:
		message = "Bot won !" if ((play_first and winner == 1) or (not play_first and winner == 2)) else "Player won !"
	if draw:
		message = "Game Draw !"
...
def user_click():
	...
	if(row and col and board[row-1][col-1] == 0):
		# global XO
		drawXO(row, col)
		check_win()
...
def bot_click():
	board_flatten = [element for sublist in board for element in sublist]
	a = int(max(pi[str(board_flatten)], key=lambda k: pi[str(board_flatten)][k]))
	row = (a // 3) + 1
	col = (a % 3) + 1
	drawXO(row, col)
	check_win()
